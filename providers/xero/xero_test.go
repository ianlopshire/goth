...
func xeroProviderSP(sp oauth.ServiceProvider) *Provider {
	return newWithSP(os.Getenv("XERO_KEY"), os.Getenv("XERO_SECRET"), "/foo", sp)
}

func mockXero(t *testing.T) (vider *Provider, stop func()) {
	p := pat.New()
	...
	xv := xeroProviderSP(oauth.ServiceProvider{
		RequestTokenUrl:   ts.URL + "/oauth/RequestToken",
		AuthorizeTokenUrl: ts.URL + "/oauth/Authorize",
		AccessTokenUrl:    ts.URL + "/oauth/AccessToken",
	})
	xv.endpointProfile = ts.URL + "/api.xro/2.0/"
